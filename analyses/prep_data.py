...
def get_exp1_nt(path):
    ...
    exp1_nt = pd.DataFrame(json.load(open(os.path.join(DATA_DIRECTORY, path, "navtrials.json"), 'r')))
...
@lru_cache()
def get_exp1_navdist(path):
    exp1_nt = get_exp1_nt(path)
...
@lru_cache()
def get_exp1_navdist_timestep(path):
    exp1_nt = get_exp1_nt(path)
...
@lru_cache()
def get_exp1_at(path):
    print("Loading Experiment 1 Attention Trials")
    exp1_at = pd.DataFrame(json.load(open(os.path.join(DATA_DIRECTORY, path, "attentiontrials.json"), 'r')))
    ...
    exp1_navdist = get_exp1_navdist(path)
    exp1_navdist_timestep = get_exp1_navdist_timestep(path)
    exp1_at = exp1_at.\
